    ...
    public class ProfileManagementHandler
    {
        public static Dictionary<string, object> GetSipProfile(string securityToken, decimal guUserId)
        {
            ...
            try
            {
                ...
                //{
                //    Logger.Instance.LogMessage(Logger.LoggerFiles.DuoDefault, "licenceKey", exception,Logger.LogLevel.Error);
                //}

                Logger.Instance.LogMessage(Logger.LoggerFiles.DuoDefault , string.Format("Sip Info.... userName : {0}, password : {1}, displayName : {2}, authName : {3}, localPort : {4}, serverPort : {5}, hostName : {6}", userName, "password", displayName, authName, localPort, serverPort, hostName), Logger.LogLevel.Debug);
            ...
            {
                Logger.Instance.LogMessage(Logger.LoggerFiles.DuoDefault, "GetSipProfile[---ProfileManagementService---]", exception, Logger.LogLevel.Error);
                return null;
            }
        }
        ...
        public static int GetACWTime(string securityToken)
        {
            ...
            {
                Logger.Instance.LogMessage(Logger.LoggerFiles.DuoDefault, "GetACWTime", exception, Logger.LogLevel.Error);
