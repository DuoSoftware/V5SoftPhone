...
namespace DuoSoftware.DuoSoftPhone.Controllers.CallStatus
{
    ...
    public class CallAgentClintConnectedState : CallState
    {
        ...
        {
            Logger.Instance.LogMessage(Logger.LogAppender.DuoLogger3, "CallAgentClintConnectedState", new NotImplementedException("Invalid Call Status."), Logger.LogLevel.Error);
        ...
        {
            try {
                if (call != null) call.CallCurrentState = new CallConferenceState();
            }
            catch (Exception exception) { Logger.Instance.LogMessage(Logger.LogAppender.DuoLogger3, string.Empty, exception, Logger.LogLevel.Error);
                throw;
            }
        ...
        public override void OnSetStatus(Call call)
        {
            try
            {
                ...
                switch (CallAction)
                {
                    ...
                    case CallAction.EtlInProgress:
                    case CallAction.EtlCall:
                        throw new InvalidOperationException(string.Format("Invalid Call Status. OnSetStatus : {0}", CallAction));
                        break;
